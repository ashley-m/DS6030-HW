...
library(ggplot2)
data %>%
  ggplot(aes(X, Y, color = Z)) + geom_point()
```
:::
...
purrr::map_dbl(mtcars, mean) # output = named numeric vector
```
:::
...
::: {.callout-note title="Solution"}
```{r}
qunif(c(.025, .975), min = min(x), max = max(x))
```
:::
...
```{r}
#- Observed Data
n = 102  # number of samples
y = 95   # number of successes
x = c(rep(1, y), rep(0, n-y))  # observed data (order doesn't matter)

#- Bootstrap sample proportion
set.seed(9341)
M = 10000               # number of bootstrap samples
#: Alternative approach using purrr::map() instead of explicit loop
library(purrr)
set.seed(9341)
y.boot = map_dbl(1:M, ~sample(x, size=n, replace=TRUE) %>% mean)

#- visualize
ggplot(tibble(y = y.boot)) +
  geom_histogram(aes(y), binwidth = .01) +
  geom_vline(xintercept = quantile(y.boot, probs = c(.05, .95)), color="red")

#- percentile CI
CI = quantile(y.boot, probs = c(.05, .95))
```
:::
...
::: {.callout-note title="Solution"}
```{r}
library(tidyverse) # for read_csv(), tibble(), mutate(), %>%

#: Load Data
url = 'https://raw.githubusercontent.com/uvastatlab/phdplus/master/data/albemarle_homes.csv'
homes = read_csv(url)

#: Fit Model
fit = lm(totalvalue ~ condition + finsqft + city, 
         data = homes)
...
# approach 1
broom::augment(fit, newdata = pred_data)  # using augment()
```
:::
...
```{r}
# load the data
data(movies_sample, package="moderndive")

# Using formula with t.test()
t.test(rating ~ genre, 
       data = movies_sample, 
       alternative = "less")
```
